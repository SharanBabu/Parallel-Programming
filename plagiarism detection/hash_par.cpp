...
double temp[4];
  ...
  #pragma omp parallel private(ii,jj,i,j,i_max,j_max,current_i,current_j,next_i,next_j,tick,zzz) reduction (+:sim) num_threads(4) 
{
int size,id;
...
for(i=id*size;i<(id+1)*size;i++)
{
	...
	for(j=0;j<tlines2;j++)
	{
	if(algorithm(h_a[i].wsentence,h_b[j].wsentence))
	{
		no_of_match[id]+=1;
		//cout<<"\ncomparison between\t"<<h_a[i].wsentence<<"\tand\t"<<h_b[j].wsentence<<"\n";
	//	cout<<"!!!!!!!!Similarity is high for the sentence "<<i<<" of src and "<<j<<" of des!!!!!!!!!!\n"<<"done by:"<<id;
...
/*#pragma omp parallel for private(j) reduction(+: no_of_match) num_threads(4)
for(i=0;i<tlines;i++)
{
	for(j=0;j<tlines2;j++)
	{
